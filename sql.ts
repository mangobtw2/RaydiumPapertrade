...
export async function tempDeleteLastHours(hours: number): Promise<void> {
    ...
    try {
        ...
        console.log(`Deleted ${result.rowCount} trades from the last ${hours} hours (after ${new Date(timestamp).toISOString()})`);
    } catch (error) {
        await pool.query('ROLLBACK');
