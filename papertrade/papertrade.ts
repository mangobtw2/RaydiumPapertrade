...
interface PnLHistory {
    [prefixId: string]: PnLDataPoint[];
}

// Add function to load/save PnL history
async function savePnLHistory(history: PnLHistory) {
    await fs.promises.writeFile('pnl_history.json', JSON.stringify(history, null, 2));
}

async function loadPnLHistory(): Promise<PnLHistory> {
    try {
        const data = await fs.promises.readFile('pnl_history.json', 'utf8');
